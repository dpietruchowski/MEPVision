...
MEPObject::MEPObject(const MEPId& id):
    id_(id), fitness_(MEPScore()), rank_(MEPScore()),
    nClones_(new int)
{
    *nClones_ = 0;
}
...
MEPObject::MEPObject(const MEPObject& rhs):
    fitness_(MEPScore()), rank_(MEPScore())
{
...
void MEPObject::swap(MEPObject& rhs)
{
    ...
    std::swap(fitness_, rhs.fitness_);
...
bool MEPObject::operator <(const MEPObject& rhs) const
{
    return fitness_.score < rhs.fitness_.score;
}
